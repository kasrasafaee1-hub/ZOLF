...
test('boots to the day picker with all four days of the split', async () => {
  ...
  await expect(page.locator('[data-day="legs-core"]')).toContainText('Legs, Abs & Core');
  await expect(page.locator('[data-day="back-triceps"]')).toContainText('Back & Triceps');
  await expect(page.locator('[data-day="chest-delts-biceps"]')).toContainText('Chest, Shoulders & Biceps');
  await expect(page.locator('[data-day="arms-core"]')).toContainText('Arms & Abs');
});
...
test('starting a day opens the first exercise ready to log', async () => {
  ...
  await expect(page.getByRole('heading', { name: 'Legs, Abs & Core' })).toBeVisible();
...
test('ticking an empty set fills in the suggested target', async () => {
  ...
  await expect(row.locator('[data-field="reps"]')).toHaveValue('5');
});
...
test('the picker flags the next day in the rotation', async () => {
  ...
  await logSet(page, 'back-squat', 0, 185, 8);
  await page.locator('[data-action="finish"]').click();
  await expect(page.locator('[data-day="back-triceps"] .pill')).toHaveText('NEXT');
});
...
test('every day of the split can be started and finished', async () => {
  for (const day of ['legs-core', 'back-triceps', 'chest-delts-biceps', 'arms-core']) {
